...
def get_live_data(order, ts_api, route_names):
    #start another thread to get data
    t = Timer(TIMER_INTERVAL, get_live_data, args=(order + 1, ts_api, route_names,))
    t.start()
    ...
    with open(file_name + str(order) + '.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    get_live_data(1, ts_api, route_names)
