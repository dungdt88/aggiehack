...
if __name__ == '__main__':
	...
	end_node = Node(-1, "End at MSC", 30.61393756, -96.33965217) 
	goal_node = Node(-1, "End at Wisenbaker", 30.62095300, -96.33822100)
	my_node = Node(-1, "Start from ETB", 30.630231, -96.338425) #ETB
	home_node = Node(-1, "home", 30.638226, -96.32246)
	start_time = datetime.datetime(2013, 11, 10, 10, 4, 0, 0)
	...
	start = clock();
	calculator = RouteCalculator(home_node, end_node, start_time)
	path = calculator.a_search()
	end = clock();
	print "Finish searching in %6.3f seconds" % (end - start)	
	print path
